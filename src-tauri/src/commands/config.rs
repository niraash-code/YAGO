...
pub async fn get_app_config(state: State<'_, AppState>) -> Result<AppConfig, String> {
    let config: tokio::sync::MutexGuard<'_, AppConfig> = state.app_config.lock().await;
    Ok(config.clone())
}
...
pub async fn update_app_config(
    ...
    let mut current: tokio::sync::MutexGuard<'_, AppConfig> = state.app_config.lock().await;
...
pub async fn get_settings(state: State<'_, AppState>) -> Result<GlobalSettings, String> {
    let settings: tokio::sync::MutexGuard<'_, GlobalSettings> = state.global_settings.lock().await;
    Ok(settings.clone())
}
...
pub async fn update_settings(
    ...
    let mut current_settings: tokio::sync::MutexGuard<'_, GlobalSettings> =
    ...
    let mut librarian: tokio::sync::MutexGuard<'_, storage::Librarian> =
