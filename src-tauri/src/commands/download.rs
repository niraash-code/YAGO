use crate::AppState;
use storage::models::InstallStatus;
use sync::orchestrator::ProgressDetailed;
use sync::{ChunkOrchestrator, OrchestratorEvent, SophonClient};
...
pub async fn start_game_download(
    ...
    let (install_path, template) = {
        let templates: tokio::sync::MutexGuard<
            '_,
    ...
    let selected_manifests_clone = selected_manifests.clone();
        ...
        while let Some(event) = rx_events.recv().await {
            match event {
                ...
                OrchestratorEvent::Completed => {
                    ...
                    // Commit Version to Disk
                    let app_state: State<'_, AppState> = app_clone.state();
                    let dbs: tokio::sync::MutexGuard<
                        '_,
                        std::collections::HashMap<String, storage::LibraryDatabase>,
                    > = app_state.game_dbs.lock().await;
                    if let Some(db) = dbs.get(&game_id_clone) {
                        if let Some(config) = db.games.get(&game_id_clone) {
                            if let Err(e) = sync::verifier::Verifier::commit_version(
                                &config.install_path,
                                &build.version,
                                &selected_manifests_clone,
                            ) {
                                eprintln!("Sophon: Failed to commit version to disk: {}", e);
                            } else {
                                println!("Sophon: Version committed to disk successfully.");
                            }
                        }
                    }
                    drop(dbs);
                    ...
                    let _ = crate::commands::library::sync_game_assets(
                        app_clone.clone(),
                        app_clone.state(),
                        game_id_clone.clone(),
                    )
...
pub async fn repair_game(
    ...
    let deep_verification = deep.unwrap_or(true);
    println!(
        "Sophon: [REPAIR] requested for game: {} (Deep: {})",
        game_id, deep_verification
    );
    ...
    let app_clone = app.clone();
    let game_id_clone = game_id.clone();
    let target_version_clone = build.version.clone();

    tauri::async_runtime::spawn(async move {
        let (tx_events, mut rx_events) = mpsc::channel(100);
        let orchestrator_handle = tokio::spawn(async move {
            orchestrator
                .verify_and_repair(tx_events, rx_pause, deep_verification)
                .await
        });
        ...
        while let Some(event) = rx_events.recv().await {
            match event {
                ...
                OrchestratorEvent::Completed => {
                    ...
                    // Commit Version to Disk
                    let app_state: State<'_, AppState> = app_clone.state();
                    let dbs = app_state.game_dbs.lock().await;
                    if let Some(db) = dbs.get(&game_id_clone) {
                        if let Some(config) = db.games.get(&game_id_clone) {
                            if let Err(e) = sync::verifier::Verifier::commit_version(
                                &config.install_path,
                    ...
                    let _ = update_game_status_internal(
                        ...
                        &game_id_clone,
                        InstallStatus::Installed,
                        Some(target_version_clone.clone()),
                    )
                    .await;
