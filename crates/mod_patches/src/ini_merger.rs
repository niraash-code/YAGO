...
impl IniMerger {
    /// Merges multiple skin INIs into a single main INI with cycle logic.
    ...
    pub fn merge_skins(character_dir: PathBuf, skins: Vec<SkinMetadata>) -> Result<()> {
        let mut main_overrides: HashMap<String, Vec<String>> = HashMap::new(); // Hash -> [LogicBody]
        let mut main_resources: Vec<String> = Vec::new();
        ...
        let merged_content = Self::generate_main_ini(skins.len(), main_overrides, main_resources);
    ...
    fn process_ini_content(
        ...
        ref_regex: &Regex,
        hash_regex: &Regex,
        main_overrides: &mut HashMap<String, Vec<String>>,
        main_resources: &mut Vec<String>,
    ) {
        ...
        for line in content.lines() {
            ...
            if trimmed.starts_with('[') && trimmed.ends_with(']') {
                ...
                if !is_override {
                    ...
                    main_resources.push(format!("[{}]", namespaced_name));
                }
            } else {
                // Process Line
                ...
                if is_override {
                    ...
                    processed_line = filename_regex
                        .replace_all(&processed_line, |c: &regex::Captures| {
                            format!("filename = Skin_{}\\{}", index, &c[1].replace("/", "\\"))
                        })
                        .to_string();
                    main_resources.push(processed_line);
                }
            }
        }
