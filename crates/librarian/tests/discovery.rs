...
async fn test_discovery_with_template_settings() {
    ...
    // ID should be "testgame.exe" (lowercase exe name)
...
fn test_scan_deep_nesting() {
    let dir = tempdir().unwrap();
    let deep_path = dir.path().join("a/b/c/d");
    ...
    let results = scanner::scan_roots(&templates, vec![dir.path().to_path_buf()], 5);
...
async fn test_game_discovery() {
    ...
    std::fs::create_dir(librarian.games_root.join("game1.exe")).unwrap();
    std::fs::create_dir(librarian.games_root.join("game2.exe")).unwrap();
    // Create a non-dir file (should be ignored)
    File::create(librarian.games_root.join("random.txt")).unwrap();
