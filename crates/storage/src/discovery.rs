...
impl Discovery {
    ...
    pub async fn add_game_by_path(
        ...
        if !db_path.exists() {
            ...
            let size_bytes = vfs::Safety::get_dir_size(&install_dir).unwrap_or(0);
            ...
            let config = GameConfig {
                ...
                cover_image: template
                    .map(|t| librarian.resolve_template_asset(&t.cover_image))
                    .unwrap_or("".to_string()),
                icon: template
                    .map(|t| librarian.resolve_template_asset(&t.icon))
                    .unwrap_or("".to_string()),
                ...
                sandbox: crate::models::SandboxConfig {
                    registry_keys: vec![],
