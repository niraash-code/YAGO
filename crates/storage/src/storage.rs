...
pub struct GamePaths {
    pub root: PathBuf,
    pub mods: PathBuf,
    pub profile_mods: PathBuf, // Profile-specific mods storage (Source of truth)
    pub db: PathBuf,
...
impl Librarian {
    ...
    pub fn game_paths(&self, game_id: &str) -> GamePaths {
        ...
        let db = self.load_game_db_sync(game_id);
        let active_profile_id = db
            .games
            .get(game_id)
            .map(|c| c.active_profile_id.clone())
            .unwrap_or_default();
        let p_uuid = Uuid::parse_str(&active_profile_id).unwrap_or_default();

        GamePaths {
            root: root.clone(),
            mods: self.mods_root.join(game_id),
            profile_mods: self.get_profile_data_dir(game_id, &p_uuid).join("Mods"),
    ...
    /// Helper for sync path resolution
    fn load_game_db_sync(&self, game_id: &str) -> LibraryDatabase {
        let db_path = self.games_root.join(game_id).join("game.json");
        if db_path.exists() {
            if let Ok(content) = std::fs::read_to_string(db_path) {
                if let Ok(db) = serde_json::from_str::<LibraryDatabase>(&content) {
                    return db;
                }
            }
        }
    ...
    pub fn resolve_template_asset(&self, name: &str) -> String {
        if name.is_empty() || name.starts_with("http") || name.starts_with("yago-asset://") {
            return name.to_string();
        }
        let path = self.templates_root.join(name);
        let path_str = path.to_string_lossy().to_string();
        format!("yago-asset://localhost/{}", urlencoding::encode(&path_str))
    }
    ...
    pub async fn load_game_db(&self, game_id: &str) -> Result<LibraryDatabase> {
        ...
        let mut db = if paths.db.exists() {
            let content = match fs::read_to_string(&paths.db).await {
                Ok(c) => c,
                Err(e) => {
                    eprintln!(
                        "Librarian ERROR: Failed to read game.json for {}: {}",
                        game_id, e
                    );
                    // Try backup
                    let bak_path = paths.db.with_extension("json.bak");
                    if bak_path.exists() {
                        println!(
                            "Librarian: Attempting to recover from backup for {}...",
                            game_id
                        );
                        fs::read_to_string(&bak_path).await?
                    } else {
                        return Err(e.into());
                    }
                }
            ...
            match serde_json::from_str::<LibraryDatabase>(&content) {
                Ok(db) => db,
                Err(e) => {
                    eprintln!(
                        "Librarian ERROR: game.json corrupted for {}: {}",
                        game_id, e
                    );
                    // Try backup
                    let bak_path = paths.db.with_extension("json.bak");
                    if bak_path.exists() {
                        println!("Librarian: Recovering from backup due to corruption...");
                        let bak_content = fs::read_to_string(&bak_path).await?;
                        serde_json::from_str::<LibraryDatabase>(&bak_content).map_err(|_| {
                            LibrarianError::Io(std::io::Error::new(
                                std::io::ErrorKind::InvalidData,
                                "Both game.json and backup are corrupted".to_string(),
                            ))
                        })?
                    } else {
    ...
    async fn write_atomic(path: &std::path::Path, content: String) -> Result<()> {
        let temp_path = path.with_extension("tmp");
        fs::write(&temp_path, content).await?;

        // If original exists, create a backup first
        if path.exists() {
